...
WHERE table_schema = 'staging' AND table_name = 'mesures_debit_sources'
ORDER BY ordinal_position;

-- 4. Qualité valeurs pour debit
SELECT COUNT(*) FILTER (WHERE "debit" IS NULL) AS nb_null, COUNT(*) AS volume_total
FROM "staging"."mesures_debit_sources";

-- 5. Gabarit futur après validation, à compléter avant exécution
-- BEGIN;
-- INSERT INTO <schema_cible>.<table_cible> (...)
-- SELECT ... FROM "staging"."mesures_debit_sources"

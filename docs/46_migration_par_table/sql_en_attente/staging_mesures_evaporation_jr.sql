...
WHERE table_schema = 'staging' AND table_name = 'mesures_evaporation_jr'
ORDER BY ordinal_position;

-- 4. Qualité valeurs pour val_evaporation
SELECT COUNT(*) FILTER (WHERE "val_evaporation" IS NULL) AS nb_null, COUNT(*) AS volume_total
FROM "staging"."mesures_evaporation_jr";

-- 5. Gabarit futur après validation, à compléter avant exécution
-- BEGIN;
-- INSERT INTO <schema_cible>.<table_cible> (...)
-- SELECT ... FROM "staging"."mesures_evaporation_jr"

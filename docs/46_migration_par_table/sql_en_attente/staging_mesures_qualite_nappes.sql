...
WHERE table_schema = 'staging' AND table_name = 'mesures_qualite_nappes'
ORDER BY ordinal_position;

-- 3. Paramètres distincts via parametre_qualite
SELECT "parametre_qualite" AS parametre, COUNT(*) AS volume
FROM "staging"."mesures_qualite_nappes"
GROUP BY "parametre_qualite"
ORDER BY volume DESC;

-- 4. Qualité valeurs pour val_qual_nap
SELECT COUNT(*) FILTER (WHERE "val_qual_nap" IS NULL) AS nb_null, COUNT(*) AS volume_total
FROM "staging"."mesures_qualite_nappes";

-- 5. Gabarit futur après validation, à compléter avant exécution
-- BEGIN;
-- INSERT INTO <schema_cible>.<table_cible> (...)
-- SELECT ... FROM "staging"."mesures_qualite_nappes"

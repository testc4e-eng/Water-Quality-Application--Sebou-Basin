-- 02_drop_et_recreation.sql
-- Reconstruction de la carte métier avec séparation stricte QUALITÉ / POLLUTION
...
WITH station_qualite AS (
    SELECT 
        ...
        'qualite.mesure_qualite_riviere' AS source_table
    ...
    GROUP BY 1, 2, 3, 4, 5, 6, 11, 12, 13, 14, 15, 16
),
station_sentinelle AS (
    SELECT 
        'STATION_SENTINELLE' AS support_type,
        'QUALITE' AS domain,
        'PHYSICO_CHIMIE' AS subdomain,
        trim(m.parametre_qualite) AS parameter_code,
        COALESCE(ref.libelle, trim(m.parametre_qualite)) AS parameter_label,
        ...
        true AS recommended_v1,
        COALESCE(s.bassin_nom, 'Sebou') AS authorized_basin,
        'qualite.mesure_qualite_sebou' AS source_table
    FROM qualite.mesure_qualite_sebou m
    JOIN api.v_station_dimension s ON s.station_id::text = m.station_id::text
    LEFT JOIN metadata.referentiel_parametre ref ON trim(ref.code_canonique) = trim(m.parametre_qualite)
    WHERE s.geom IS NOT NULL
    GROUP BY 1, 2, 3, 4, 5, 6, 11, 12, 13, 14, 15, 16
),
station_hydro AS (
    SELECT 
        ...
        true AS recommended_v1,
        COALESCE(s.bassin_nom, 'Sebou') AS authorized_basin,
        'hydro.mesure_debit' AS source_table
...
barrage_hydro AS (
    SELECT 
        ...
        'hydro.mesure_barrage_param' AS source_table
...
station_meteo AS (
    SELECT 
        ...
        true AS recommended_v1,
        COALESCE(s.bassin_nom, 'Sebou') AS authorized_basin,
        'meteo.mesure_precipitation' AS source_table
    FROM meteo.mesure_precipitation m
    JOIN api.v_station_dimension s ON s.station_id::text = m.station_id::text
    WHERE s.geom IS NOT NULL
    GROUP BY 1, 2, 3, 4, 5, 6, 11, 12, 13, 14, 15, 16
),
point_prelevement_pollution AS (
    SELECT 
        'POINT_PRELEVEMENT_POLLUTION' AS support_type,
        'POLLUTION' AS domain,
        'PRELEVEMENT' AS subdomain,
        trim(m.param_code_legacy) AS parameter_code,
        trim(m.param_code_legacy) AS parameter_label,
        NULL AS unit,
        count(DISTINCT p.id) AS object_count,
        count(m.*) AS measure_count,
        min(COALESCE(p.date_prelevement, p.date_reception)) AS date_min,
        max(COALESCE(p.date_prelevement, p.date_reception)) AS date_max,
        true AS has_geometry,
        true AS has_timeseries,
        false AS has_thresholds,
        true AS recommended_v1,
        'Sebou' AS authorized_basin,
        'qualite.source_pollution_mesure_param' AS source_table
...
source_pollution AS (
    SELECT 
        ...
        'api.v_pollution_sites' AS source_table
...
SELECT 
    ...
    bassin_nom,
    sous_bassin_nom,
    jsonb_build_object('type_station', type_station, 'source_table', 'qualite.mesure_qualite_riviere', 'source_kind', 'suivi_qualite_abh', 'data_family', 'qualite_eau') AS attributes,
    true AS recommended_v1
FROM api.v_station_dimension
WHERE geom IS NOT NULL 
  AND code_station NOT IN (SELECT site_code FROM api.v_pollution_sites WHERE site_code IS NOT NULL)
  AND station_id::text IN (SELECT DISTINCT station_id::text FROM qualite.mesure_qualite_riviere)
UNION ALL
SELECT 
    'STATION_SENTINELLE' AS support_type,
    station_id::text AS object_id,
    ...
    bassin_nom,
    sous_bassin_nom,
    jsonb_build_object('type_station', type_station, 'source_table', 'qualite.mesure_qualite_sebou', 'source_kind', 'sentinelle_qualite_abh', 'data_family', 'qualite_eau') AS attributes,
    true AS recommended_v1
FROM api.v_station_dimension
WHERE geom IS NOT NULL
  AND station_id::text IN (SELECT DISTINCT station_id::text FROM qualite.mesure_qualite_sebou)
UNION ALL
SELECT 
    'STATION_HYDRO' AS support_type,
    station_id::text AS object_id,
    ...
    bassin_nom,
    sous_bassin_nom,
    jsonb_build_object('type_station', type_station, 'source_table', 'hydro.mesure_debit', 'data_family', 'hydrologie') AS attributes,
    true AS recommended_v1
FROM api.v_station_dimension
...
SELECT 
    ...
    bassin_nom,
    sous_bassin_nom,
    jsonb_build_object('type_station', type_station, 'source_table', 'meteo.mesure_precipitation', 'data_family', 'climatologie') AS attributes,
    true AS recommended_v1
FROM api.v_station_dimension
...
SELECT 
    ...
    bassin_nom,
    sous_bassin_nom,
    jsonb_build_object('nom_oued', nom_oued, 'type_barrage', type_barrage, 'statut', statut, 'source_table', 'hydro.mesure_barrage_param', 'data_family', 'hydrologie') AS attributes,
    true AS recommended_v1
...
SELECT 
    ...
    COALESCE(p.point_prelevement, 'Prélèvement ' || p.id::text) AS object_name,
    p.id::text AS object_code,
    ST_SetSRID(ST_MakePoint(p.coord_x, p.coord_y), 4326) AS geom,
    ST_AsGeoJSON(ST_SetSRID(ST_MakePoint(p.coord_x, p.coord_y), 4326), 6)::jsonb AS geometry_geojson,
    p.abh AS bassin_nom,
    NULL AS sous_bassin_nom,
    jsonb_build_object('source_table', 'qualite.source_pollution_mesure_param', 'source_kind', 'prelevement_pollution', 'data_family', 'pollution') AS attributes,
    true AS recommended_v1
FROM qualite.source_pollution_prelevement p
WHERE p.coord_x IS NOT NULL AND p.coord_y IS NOT NULL
UNION ALL
SELECT 
    'SOURCE_POLLUTION' AS support_type,
    ...
    NULL AS sous_bassin_nom,
    jsonb_build_object('commune', commune, 'source_type_label', source_type_label, 'source_table', 'api.v_pollution_sites', 'source_kind', 'site_pollution', 'data_family', 'pollution') AS attributes,
    true AS recommended_v1
...
prelevement_pollution AS (
    SELECT DISTINCT ON (p.id, trim(m.param_code_legacy))
        'POINT_PRELEVEMENT_POLLUTION' AS support_type,
        p.id::text AS object_id,
        trim(m.param_code_legacy) AS parameter_code,
        COALESCE(p.date_prelevement, p.date_reception) AS last_date,
        m.valeur_num AS last_value,
        NULL AS quality_flag
    FROM qualite.source_pollution_mesure_param m
    JOIN qualite.source_pollution_prelevement p ON p.id = m.prelevement_id
    WHERE m.valeur_num IS NOT NULL
    ORDER BY p.id, trim(m.param_code_legacy), COALESCE(p.date_prelevement, p.date_reception) DESC
),
pollution AS (

...
|---|---|---:|---|---|---|---|---|---|
| `meteo.mesure_temperature` | météo | 0 | non documenté audit | `STATION_METEO` | donnée absente actuelle | `api.v_meteo_temperature` | vue vide / pipeline manquant | `PIPELINE_TO_IMPLEMENT` |
...
| `hydro.mesure_debit` | hydrologie | 521433 | non documenté audit | `RIVIERE` | exploitable | `api.v_hydro_debit` cible | volumétrie élevée | `API_TARGET` |
...
| `qualite.mesure_qualite_barrage` | qualité barrage | 15808 | non documenté audit | `BARRAGE` | `parametre_ref_id` absent en branche historique | `api.v_barrage_qualite`, `api.v_qualite_terrain` | jointure par code requise | `SOURCE_READY_WITH_JOIN_RULES` |
...
| `qualite.suivi_qualite_barrage_garde_hebdo` | qualité barrage | 7094 | non documenté audit | `BARRAGE` | `MO_METAL` déjà résolu vers `Mo` sur 11 lignes | vues qualité spécialisées, barrage qualité | double classification possible | `CLOTURE_C4E_COMPLETE` |

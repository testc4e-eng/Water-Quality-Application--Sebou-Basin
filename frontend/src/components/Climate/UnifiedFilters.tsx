...
export default function UnifiedFilters({ theme, onChange, compact }: Props) {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    getHierarchyParameters(theme, selectedSubmenu)
      .then((rows) => {
        if (cancelled) return;
        setParameters(rows || []);
        setSelectedParamCode(undefined);
  ...
  useEffect(() => {
    ...
    const key = JSON.stringify({
      ...
      paramCode: payload.parameter?.param_code ?? null,
    });
    if (lastPayloadRef.current === key) return;
    lastPayloadRef.current = key;
    onChangeRef.current(payload);
  }, [selectedSubmenu, selectedParamCode, selectedEntityId, parameters]);

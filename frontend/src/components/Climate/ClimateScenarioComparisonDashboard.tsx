...
function Select({
  ...
  return (
    <div className="relative">
      <select
        className="w-full appearance-none rounded-md border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700 outline-none transition focus:border-sky-400 focus:ring-2 focus:ring-sky-100 disabled:bg-slate-100 disabled:text-slate-400"
        value={value ?? ""}
...
export default function ClimateScenarioComparisonDashboard() {
    ...
    return Array.from(
      new Set(
        rowsStats
          ...
          .map((row) => String(row.property_name))
      )
    ).filter(Boolean);
        ...
        rowsStats
          .filter(
            (row) =>
              ...
              String(row.property_name) === String(variable || "")
          )
          .map((row) => String(row.time_step))
      )
    ).filter(Boolean);
  }, [rowsStats, sourceType, variable]);
  ...
  }, [rowsStats, sourceType, variable, aggregation]);

  useEffect(() => {
    setReferenceKey(undefined);
    setSelectedScenarioKeys([]);
  }, [sourceType, variable, aggregation]);
      ...
      const nextSeries = await Promise.all(
        allKeys.map(async (key, index) => {
          ...
          return {
            ...
            variable: String(row.property_name),
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                ...
                <div className="space-y-1">
                  ...
                  <Select value={variable} onChange={setVariable} disabled={!sourceType} placeholder="Choisir un parametre...">
                    {variables.map((item) => (
                      <option key={item} value={item}>
                        {item}
                      </option>
              ...
              <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                <div className="space-y-1">
                  ...
                  <Select value={aggregation} onChange={setAggregation} disabled={!variable} placeholder="Choisir une aggregation...">
              ...
              {stationName && variable && aggregation ? (
                <div className="rounded-lg border border-sky-200 bg-sky-50 px-3 py-2 text-xs text-sky-700">
                  Reference et comparaison pour <span className="font-semibold">{stationName}</span> • {variable} • {formatAggregationLabel(aggregation)}
                </div>
              ) : null}
            </div>
          </div>

...
export default function HydroScenarioComparisonDashboard() {
  ...
  const summaryRows = useMemo(() => {
    const referenceSerie = scenarioSeries.find((serie) => serie.key === referenceKey);
    const referenceValues = referenceSerie?.points.map((point) => point.value).filter((value) => !Number.isNaN(value)) || [];
    const referenceMean = referenceValues.length
    ...
    return scenarioSeries.map((serie) => {
      ...
      const deltaMean = referenceMean == null || serie.key === referenceKey ? 0 : mean - referenceMean;
      return {
        role: serie.key === referenceKey ? "Référence" : "Comparaison",
        ...
        deltaMean: Number(deltaMean.toFixed(2)),
      };
    });
  }, [scenarioSeries, referenceKey]);

  const detailRows = useMemo(() => {
    const referenceSerie = scenarioSeries.find((serie) => serie.key === referenceKey);
    const referenceByDate = new Map((referenceSerie?.points || []).map((point) => [point.date, point.value]));

    return scenarioSeries
      .flatMap((serie) =>
        serie.points.map((point) => {
          const refValue = referenceByDate.get(point.date);
          const delta = refValue == null || serie.key === referenceKey ? null : point.value - refValue;
          return {
            date: point.date,
            scenario: serie.label,
            role: serie.key === referenceKey ? "Référence" : "Comparaison",
                    ...
                    >
                      <div className="mb-1 flex items-center justify-between gap-2">
                        <span className={`text-xs font-semibold uppercase tracking-wider ${isReference ? "text-blue-700" : isSelected ? "text-emerald-700" : "text-slate-500"}`}>
                          {isReference ? "Référence" : isSelected ? "Comparaison" : "Cliquer pour comparer"}
                        </span>
            ...
            {summaryRows.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={summaryRows.map((row) => ({ ...row, name: row.scenario }))}>
                  ...
                  <Line dataKey="deltaMean" stroke="#0f172a" name="Delta moyen vs référence" strokeDasharray="4 4" strokeWidth={2} />
                </LineChart>
              </ResponsiveContainer>
            ) : (
              <div className="flex h-full items-center justify-center rounded-xl bg-slate-50 text-sm text-slate-500">
              ...
              <table className="min-w-full divide-y divide-slate-200 text-xs">
                <thead className="sticky top-0 bg-slate-50 text-slate-600">
                  <tr>
                    ...
                    <th className="px-3 py-2 text-right font-semibold">Delta vs réf.</th>
                  </tr>
                </thead>
                  ...
                  {detailRows.map((row, index) => (
                    <tr key={`${row.scenario}-${row.date}-${index}`}>
                      ...
                      <td className={`px-3 py-2 text-right font-semibold ${
                        row.delta == null ? "text-slate-400" : row.delta >= 0 ? "text-emerald-700" : "text-rose-700"
                      }`}>

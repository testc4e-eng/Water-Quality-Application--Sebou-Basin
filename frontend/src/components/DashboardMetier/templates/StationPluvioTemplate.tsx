...
export function StationPluvioTemplate({
  properties,
  compact,
}: {
  ...
  const rainfall = findLatestValue(values, "PLUIE") || values[0];
  const temperature = findLatestValue(values, "TEMPERATURE_AIR") || findLatestValue(values, "T_AIR");

  if (values.length === 0) {
    return <div className="text-xs text-slate-500">Aucune donnée pluviométrique récente.</div>;
  }

  return (
    <div className="space-y-1.5">
      <KpiItem icon={CloudRain} label="Pluie" value={rainfall} colorClass="text-cyan-600" compact={compact} />
      {!compact && temperature && (

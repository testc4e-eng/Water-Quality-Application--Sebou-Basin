...
export function BusinessPopup({ properties, variant = "popup", onClose }: BusinessPopupProps) {
  ...
  const summaryValue = latestValues[0] ?? null;
  const isCompact = variant === "compact";

  if (isCompact) {
    return (
      <div className="w-[280px] rounded-2xl border border-slate-200 bg-white/96 p-3 shadow-[0_16px_34px_rgba(15,23,42,0.14)] backdrop-blur">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <div className="truncate text-sm font-semibold leading-tight text-slate-950">{title}</div>
            <div className="mt-0.5 truncate text-[11px] text-slate-500">{renderLocation(properties)}</div>
          </div>
          {onClose ? (
            <button
              type="button"
              ...
              aria-label="Fermer le panneau entité"
            >
        ...
        <div className="mt-2 flex flex-wrap items-center gap-1.5">
          ...
          {dataStatus ? (
            <span className={`rounded border px-1.5 py-0 text-[10px] font-semibold ${dataStatusTone(dataStatus)}`}>
              {dataStatus}
            </span>
          ) : null}
        </div>
        ...
        <div className="mt-2 grid gap-2">
          <div className="rounded-xl border border-slate-200 bg-slate-50 px-2.5 py-2">
            <div className="text-[9px] font-semibold uppercase tracking-[0.14em] text-slate-400">Description</div>
            <div className="mt-1 text-[11px] leading-5 text-slate-700">
              {typeLabel}
              {properties.bassin || properties.sous_bassin_nom ? ` • ${String(properties.bassin || properties.sous_bassin_nom)}` : ""}
              {properties.last_measure_date || properties.date_max ? ` • Maj ${String(properties.last_measure_date || properties.date_max)}` : ""}
            </div>
          </div>
          ...
          <div className="rounded-xl border border-slate-200 bg-white px-2.5 py-2">
            <div className="flex items-center justify-between gap-3">
              <div>
                <div className="text-[9px] font-semibold uppercase tracking-[0.14em] text-slate-400">Mesure clé</div>
                <div className="mt-1 text-[11px] font-medium text-slate-800">
                  {summaryValue ? latestValueLabel(summaryValue) : "Aucune mesure disponible"}
                </div>
              </div>
              {summaryValue ? classBadge(summaryValue) : null}
            </div>
          </div>

          <div className="grid grid-cols-2 gap-2 text-[11px]">
            <div className="rounded-xl border border-slate-200 bg-white px-2.5 py-2">
              <div className="text-[9px] font-semibold uppercase tracking-[0.14em] text-slate-400">Paramètres</div>
              <div className="mt-1 font-semibold text-slate-900">{String(properties.parameter_count || latestValues.length || 0)}</div>
            </div>
            <div className="rounded-xl border border-slate-200 bg-white px-2.5 py-2">
              <div className="text-[9px] font-semibold uppercase tracking-[0.14em] text-slate-400">Mesures</div>
              <div className="mt-1 font-semibold text-slate-900">{String(properties.measure_count || 0)}</div>
            </div>
          </div>
        </div>

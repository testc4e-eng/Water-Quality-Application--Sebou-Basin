...
def get_column_type(fullname: str, column_name: str) -> Optional[str]:
    """
    Returns the normalized SQL type for a given column using information_schema.
    Useful to distinguish real PostGIS columns from JSON columns named "geometry".
    """
    schema, table = _split_table(fullname)
    q = """
    SELECT lower(COALESCE(udt_name, data_type))
    FROM information_schema.columns
    WHERE table_schema = %s
      AND table_name = %s
      AND column_name = %s
